...
// ── FLASH SALE SYSTEM — fetches live from Apps Script ────────────
// Updated to Cloudflare Worker endpoint (replaces Google Apps Script)
const SGTB_SCRIPT_URL = 'https://island-guide-backend.the-sg-travel-buddy.workers.dev/flash-sales';
const FS_DONT_SHOW_KEY = 'sgtb_no_flash_deals';
...
// Fetch live flash sales from Apps Script (Google Sheets)
async function fsLoadSales() {
  try {
    const res = await fetch(`${SGTB_SCRIPT_URL}?action=getFlashSales`);
    const data = await res.json();
    if (data.ok && data.sales) {
      const today = fsTodaySGT();
      fsSales = data.sales.filter(s => s.active && s.usedBy >= today);
    } else {
      fsSales = [];
    }
  } catch(e) {
...
function fsRenderSlides() {
  ...
  slidesEl.innerHTML = fsSales.map((sale, i) => {
    const displayPrice  = parseFloat(sale.displayPrice || 0).toFixed(2);
    const standardPrice = sale.standardPrice ? parseFloat(sale.standardPrice).toFixed(2) : null;
    const savingPct     = sale.savingPct || null;
    const displayName   = sale.customAttraction || sale.attraction || '';
    return `
    <div class="fs-slide">
      <div class="fs-slide-attraction">${displayName}</div>
      <div class="fs-slide-meta">
        ...
        ${savingPct ? `<span class="fs-slide-badge" style="background:rgba(5,150,105,0.18);color:#064e3b;font-weight:800;">💚 Save ${savingPct}%</span>` : ''}
      </div>
      ...
      <div style="margin-top:12px;">
        <button onclick="fsOpenMessenger()" style="width:100%;padding:10px 0;background:linear-gradient(135deg,#1877F2,#0a5bbf);border:none;border-radius:10px;color:#fff;font-family:inherit;font-weight:800;font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:7px;">
          <svg width="18" height="18" viewBox="0 0 36 36" fill="white"><path d="M18 2C9.16 2 2 8.71 2 17c0 4.68 2.19 8.87 5.63 11.72V34l5.13-2.82C14.38 31.67 16.15 32 18 32c8.84 0 16-6.71 16-15S26.84 2 18 2z"/><path d="M19.79 21.69l-4.07-4.34-7.95 4.34 8.74-9.28 4.17 4.34 7.85-4.34z" fill="#1877F2"/></svg>
  ...
  // Inject dont-show checkbox if not already there
  const box = document.getElementById('flashSaleBox');
  if (box && !document.getElementById('fsDontShowWrap')) {
    const wrap = document.createElement('div');
    wrap.id = 'fsDontShowWrap';
    wrap.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 16px 4px;border-top:1px solid rgba(0,0,0,0.07);margin-top:8px;';
    wrap.innerHTML = `
      <input type="checkbox" id="fsDontShowChk" style="width:16px;height:16px;accent-color:#F59E0B;cursor:pointer;">
      <label for="fsDontShowChk" style="font-size:12px;font-weight:600;color:#6B7280;cursor:pointer;user-select:none;">Don't show me deals again on this device</label>`;
...
function fsGoTo(idx) {
  ...
  document.querySelectorAll('.fs-dot').forEach((d, i) => d.classList.toggle('active', i === idx));
  document.getElementById('fsBannerCounter').textContent =
    fsSales.length > 1 ? `${idx+1} of ${fsSales.length}` : '1 deal';
  fsUpdateNav();
}

function fsNav(dir) { fsGoTo(fsCurrentIdx + dir); }
...
function fsOpenMessenger() {
  const appLink = 'fb-messenger://user-thread/115403788266848';
  const webLink = 'https://m.me/115403788266848';
  const start = Date.now();
  window.location.href = appLink;
  setTimeout(() => { if (Date.now() - start < 2000) window.open(webLink, '_blank'); }, 1000);
}

async function openFlashBanner() {
  // Always fetch fresh data before showing
  try {
    const res = await fetch(SGTB_SCRIPT_URL + '?action=getFlashSales');
    const data = await res.json();
    if (data.ok && data.sales) {
      const today = new Date(Date.now() + 8*3600000).toISOString().slice(0,10);
      fsSales = data.sales.filter(s => s.usedBy >= today);
    }
  } catch(e) { fsSales = []; }
...
function closeFlashBanner() {
  // Check dont-show checkbox
  const chk = document.getElementById('fsDontShowChk');
  if (chk && chk.checked) {
    localStorage.setItem(FS_DONT_SHOW_KEY, '1');
  }
  const overlay = document.getElementById('flashSaleOverlay');
...
async function checkAndShowFlashBanner() {
  if (fsShown) return;
  // Respect dont-show preference
  if (localStorage.getItem(FS_DONT_SHOW_KEY) === '1') return;
  await fsLoadSales();
  if (fsSales.length > 0) {
    openFlashBanner();
  }
...
document.addEventListener('DOMContentLoaded', function() {
  // Flash banner — 2 second delay to let page and protection scripts settle
  setTimeout(function() {
    const homeScreen = document.getElementById('screen-home');
    if (homeScreen && homeScreen.classList.contains('active')) {
      if (localStorage.getItem(FS_DONT_SHOW_KEY) !== '1' && !fsShown) {
        openFlashBanner();
      }
    }
  }, 2000);
